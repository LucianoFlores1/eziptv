...
export default function SeriesDetailPage() {
  ...
  const seriesIdNum = Number.parseInt(seriesId, 10)
  const hasValidId = Number.isFinite(seriesIdNum)

  const { credentials } = useAuth()
  const { toggleFavorite } = useFavorites()
  const isFav = useIsFavorite(hasValidId ? seriesIdNum : 0, 'series')

  const seriesData = useLiveQuery(
    () => (hasValidId ? db.series.get(seriesIdNum) : undefined),
    [hasValidId, seriesIdNum]
  )
  ...
  useEffect(() => {
    ...
    fetchInfo()
  }, [credentials, hasValidId, seriesIdNum])

  if (!hasValidId) {
    return (
      <div className="px-4 py-6 md:px-8">
        <p className="text-sm text-muted-foreground">Missing or invalid series id.</p>
      </div>
    )
  }
  ...
  return (
    <div className="pb-8">
      <div className="relative">
        ...
        <Link
          href={`/series/category?category=${encodeURIComponent(categoryId)}`}
