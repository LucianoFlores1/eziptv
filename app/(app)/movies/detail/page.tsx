...
export default function MovieDetailPage() {
  ...
  const vodIdNum = Number.parseInt(vodId, 10)
  const hasValidId = Number.isFinite(vodIdNum)

  const { credentials } = useAuth()
  const { toggleFavorite } = useFavorites()
  const isFav = useIsFavorite(hasValidId ? vodIdNum : 0, 'vod')
  const { position, duration, completed, progress } = useWatchedStatus(
    hasValidId ? vodIdNum : 0,
    'vod'
  )

  const movie = useLiveQuery(
    () => (hasValidId ? db.movies.get(vodIdNum) : undefined),
    [hasValidId, vodIdNum]
  )
  ...
  useEffect(() => {
    ...
    fetchDetail()
  }, [credentials, hasValidId, vodIdNum])

  if (!hasValidId) {
    return (
      <div className="px-4 py-6 md:px-8">
        <p className="text-sm text-muted-foreground">Missing or invalid movie id.</p>
      </div>
    )
  }
  ...
  return (
    <div className="pb-8">
      <div className="relative">
        ...
        <Link
          href={`/movies/category?category=${encodeURIComponent(categoryId)}`}
